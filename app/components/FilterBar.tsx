...
function FilterChip({
  ...
  return (
    <button
      type="button"
      className={cn(
        'min-h-[36px] px-3 py-1 rounded-full text-sm font-medium transition-colors whitespace-nowrap',
        active
...
export function FilterBar({
  ...
  const sortLabel =
    filters.sortDirection === 'asc' ? 'Sort: oldest first' : 'Sort: newest first'

  return (
    <div
      className={cn(
        'sticky',
        BELOW_HEADER_TOP_CLASS,
        'z-[5] bg-white border-b border-slate-200',
      )}
    >
      <div className="max-w-[960px] mx-auto px-4 py-3 space-y-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="min-h-[36px] min-w-[36px] inline-flex items-center justify-center rounded-lg border border-slate-200 text-slate-600 hover:bg-slate-100 transition-colors"
          >
          ...
          {hasActiveFilters && (
            <button
              type="button"
              onClick={clearAll}
              className="min-h-[36px] px-2 inline-flex items-center gap-1 rounded-lg text-sm text-slate-500 hover:text-slate-700 hover:bg-slate-100 transition-colors whitespace-nowrap"
            >
              <X className="h-3 w-3" />
              Clear
            </button>
